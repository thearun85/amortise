...
class LoanRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.principal <= Decimal("0"):
            raise ValueError(
                f"'principal' must be greater than zero, got {self.principal!r}"
            )
        ...
        if self.term_months <= 0:
            raise ValueError(
                f"'term_months' must be greater than zero, got {self.term_months!r}"
            )
